...
class SiameseQACNN(object):
    ...
    def _emol_input(self, input, length):
        tokens_input = input
    ...
    def _build(self):
        ...
        self.q_embed = tf.nn.dropout(self._emol_input(self._ques, q_mask), keep_prob=self.dropout_keep_prob)
        time.sleep(3)
        self.a_embed = tf.nn.dropout(self._emol_input(self._ans, a_mask), keep_prob=self.dropout_keep_prob)
        time.sleep(3)
        self.a_neg_embed = tf.nn.dropout(self._emol_input(self._ans_neg, a_mask_neg), keep_prob=self.dropout_keep_prob)
        time.sleep(3)
