...
class Dataset(object):
    ...
    def _genVocabFile(self, file_in):
        words = []
        ...
        print('alltokens总共有{}个词'.format(len(allTokens)))
...
def transform(fin_path):
    transformed_corpus = []
    with open(fin_path, 'r', encoding='utf-8') as fin:
        fin.readline()
        for line in fin:
            qid, q, aid, a, label = line.strip().split('\t')
            q = [w.lower() for w in q.split()]
            a = [w.lower() for w in a.split()]
            transformed_corpus.append([qid, q, aid, a, int(label)])
    return transformed_corpus


def transform_train(fin_path):
    transformed_corpus = []
    with open(fin_path, 'r', encoding='utf-8') as fin:
        fin.readline()
        for line in fin:
            ...
            q = [w.lower() for w in q.split()]
            a_pos = [w.lower() for w in a_pos.split()]
            a_neg = [w.lower() for w in a_neg.split()]
